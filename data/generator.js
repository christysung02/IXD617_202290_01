...
const countriestype = (type) => {
    let types = {
        'Taiwan':['sticky tofu','boba milk tea','hot pot'],
        'United States':['hamburger','fries','chicken'],
        'Japan':['sushi','miso','ramen']
    }
    return chance.pickone(types[type]);
}

//make data
//map is the function that loop the array
...
//countries
const getCountries = () => (new Array(50)).fill(0).map((o,i)=>{
    o = {};
    o.id = i + 1;
    o.user_id = chance.natural({min:1, max:10});
    o.name = chance.first();
    o.type = chance.pickone(['Taiwan','United States','Japan']);
    o.breed = countriestype(o.type);
...
window.addEventListener('DOMContentLoaded',()=>{
    ...
    document.querySelector('.countries-json').addEventListener('click',()=>{ exportAsJSON('Countries',getCountries()); });
    ...
    document.querySelector('.countries-sql').addEventListener('click',()=>{ exportAsSQL('track_ixd617_countries',getCountries()); });
